...
>
> 🔒 **访客本地端口冲突校验**（`visitor` 通道，创建/编辑均生效）：访客在本地起监听，
> STCP/XTCP 走 **TCP**、SUDP 走 **UDP**。若**任一实例**下已有**同协议族** + 同 `bindPort` +
> 地址重叠（同 IP，或任一为 `0.0.0.0`/空通配）的访客，返回
> `409 visitor_port_conflict`，`error.details` 含 `config_id` / `config_name` / `name` /
> `type` / `bind_addr` / `bind_port`。注意 STCP 与 XTCP 同属 TCP，**会互相冲突**；

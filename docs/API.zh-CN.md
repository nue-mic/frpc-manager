...
### 3.10 规则导入导出 / Rule Import-Export

把选中的代理/访客以「可移植信封 v1」或拆分 TOML 导出，在另一台机器上**先预解析（parse）再提交（import）**。三个端点都是 `POST`，均挂在需鉴权子树下（`Authorization: Bearer <token>`），路径前缀 `/api/v1/configs/{id}/proxies/`。

设计要点：**代理与访客共用一个扁平命名空间**——冲突判定（`parse` 的 `conflict`、`import` 的「同名已存在」）都基于该配置 `proxies[]` 内**所有**名称（代理 + 访客）统一比对，与 `import` 的实际落库逻辑严格对齐。

#### 3.10.1 POST `/api/v1/configs/{id}/proxies/export` — 导出选中规则

请求体（全部可选）：

| 字段 | 类型 | 默认 | 说明 |
|---|---|---|---|
| `format` | string | `toml` | `toml`（拆分 TOML 片段）或 `portable`（可移植信封 v1） |
| `kind` | string | `all` | `all` / `proxy` / `visitor`，筛选导出种类 |
| `names` | string[] |  | 按 `name` 或 range 别名筛选；空数组表示该种类全选 |
...
```json
{
  "format": "toml",
  "kind": "all",
  "counts": { "proxies": 2, "visitors": 1 },
  "proxiesToml": "[[proxies]]\nname = \"ssh\"\n...",
  "visitorsToml": "[[visitors]]\nname = \"db-visitor\"\n...",
  "filename": "frps-tokyo-rules.toml"
}
```

`portable` 形态返回（`200`）：`portableJson` 是缩进序列化后的 `PortableEnvelope` **字符串**（非内嵌对象），便于直接下载/复制：
...
```json
{
  ...
  "filename": "frps-tokyo-rules.json"
}
```

`counts` 为 `{ proxies, visitors }` 计数；`proxiesToml` / `visitorsToml` 在对应种类为空时为 `""`。配置不存在 `404`；构建信封 / 渲染 TOML 失败 `500 / internal`。

**可移植信封 v1（`PortableEnvelope`）结构**——`proxies` / `visitors` 内层为上游 frp 的 **camelCase** `v1.Typed{Proxy,Visitor}Config`：

| 字段 | 类型 | 说明 |
|---|---|---|
| `frpcManagerExport` | string | 信封标记/版本，固定 `"v1"`，`parse` 据此识别可移植格式 |
| `kind` | string | 固定 `"rules"` |
| `exportedAt` | string |  导出时间（可空） |
| `source` | object | 来源信息 `PortableSource`（见下） |
| `proxies` | array | 非访客代理（range 代理已展开） |
| `visitors` | array | 访客（`role == "visitor"`） |

**`source`（`PortableSource`）**——携带源端 frp `user`，供导入侧推导访客 `serverUser`：

| 字段 | 类型 | 说明 |
|---|---|---|
| `configId` | string | 源实例 ID |
| `configName` | string | 源实例名 |
| `user` | string | 源端 frpc 的 `user`（推导访客 `serverUser` 用） |
| `daemon` | string | 导出端守护进程版本 |
| `frp` | string | 内嵌 frp 版本 |

#### 3.10.2 POST `/api/v1/configs/{id}/proxies/parse` — 预解析（dry-run，不落库）

把一段粘贴文本**自动识别格式**并逐条预览，绝不写盘。请求体：

| 字段 | 类型 | 必填 | 说明 |
|---|---|---|---|
| `content` | string | √ | 粘贴的原始文本（可移植 JSON / TOML / INI 片段或整份 frpc 配置） |
| `format` | string |  | 接受但**忽略**，识别始终自动（portable → toml/ini） |

返回（`200`）：

```json
{
  "detectedFormat": "portable",
  "items": [
    {
      "name": "speed-test-tcp", "type": "stcp", "kind": "proxy",
      "summary": "stcp · localPort 22",
      "raw": { "name": "speed-test-tcp", "type": "stcp", "...": "..." },
      "conflict": null,
      "pairable": true,
      "suggestedVisitor": { "name": "speed-test-tcp-visitor", "type": "stcp", "...": "..." }
    }
  ],
  "source": { "configId": "...", "user": "ln2-node", "...": "..." }
}
```

| 字段 | 类型 | 说明 |
|---|---|---|
| `detectedFormat` | string | `portable` / `toml` / `ini` / `unknown` |
| `items` | array | 逐条预览（见下表） |
| `source` | object | 仅可移植/有 `user` 时出现，结构同 `PortableSource`（toml/ini 仅填 `user`） |
| `globalError` | string | **仅** `detectedFormat == "unknown"` 时出现，携带解析错误信息（此时 `items` 为 `[]`，HTTP 仍 `200`） |
...
|---|---|---|
| `name` | string | 规则名 |
| `type` | string | 规则类型（`tcp`/`stcp`/`xtcp`/…） |
| `kind` | string | `proxy` 或 `visitor` |
| `summary` | string | 人类可读摘要（访客 `… · bind addr:port`；代理 `… · localPort N`） |
| `raw` | object | 转换后的 `v1.Typed{Proxy,Visitor}Config`（camelCase），供回填编辑表单 |
| `conflict` | string\|null | 与本配置扁平命名空间同名时为 `"name_exists"`，否则 `null` |
| `error` | string | 代理无法转换时出现（如 `"无法转换该代理"`） |
| `pairable` | bool | `stcp/xtcp/sudp` 代理为 `true`，可派生配套访客 |
| `suggestedVisitor` | object | `pairable` 时给出派生的 `v1.TypedVisitorConfig`（见下「代理 → 访客派生」） |

**代理 → 访客派生（`DerivePairVisitor`）**——对每条可配对代理预生成一份配套访客：

| 派生字段 | 取值 | 说明 |
|---|---|---|
| `serverName` | = 代理 `name` | 指向对端代理名 |
| `secretKey`（`SK`） | = 代理 `secretKey` | 两端密钥一致 |
| `serverUser` | = 信封 `source.user` | 源端 frpc 的 `user` 前缀 |
| `useEncryption` / `useCompression` | = 代理同名值 | **两端强制对齐**（不一致 frp 隧道会静默失败） |
| `bindAddr` | `0.0.0.0`（默认） | 本地绑定地址 |
| `bindPort` | 后端建议的空闲端口 | 由 `SuggestVisitorBindPort`（起始 `10000`）按协议族+地址避让现有访客逐一挑选，批内不重复 |
| `protocol` | `xtcp` 时为 `quic` | 仅 XTCP P2P 协议 |

派生访客的 `name` 会避开既有规则名与本批次已选名（依次尝试 `base` → `base-visitor` → `base-visitor-2`…），确保默认 `create` 动作不撞名。

#### 3.10.3 POST `/api/v1/configs/{id}/proxies/import` — 提交解析后的批次

把前端确认过的导入项**一次性**落库（一次 `Update` → 一次热重载 → 一次 `config.changed` 事件）。请求体：

```json
{
  "items": [
    { "kind": "proxy",   "action": "create",    "proxy":   { "name": "ssh", "type": "tcp", "...": "..." } },
    { "kind": "visitor", "action": "rename", "newName": "db2", "visitor": { "name": "db", "type": "stcp", "...": "..." } }
  ]
}
```
...
| 字段 | 类型 | 必填 | 说明 |
|---|---|---|---|
| `kind` | string |  | `proxy` / `visitor`（仅语义标注；实际按 `proxy`/`visitor` 哪个非空判定） |
| `action` | string | √ | `create` / `overwrite` / `rename` / `skip` |
| `newName` | string | `rename` 时必填 | 重命名后的新名称 |
| `proxy` | object |  | `v1.TypedProxyConfig`（代理项给这个） |
| `visitor` | object |  | `v1.TypedVisitorConfig`（访客项给这个） |

`proxy` 与 `visitor` 二选一；两者都为空的项**静默跳过**（不计入结果）。

**best-effort 语义**：单条坏项记为 `failed`/`skipped`，其余继续；整调用仅在写盘致命失败时报错。若全批 `applied == 0` 则不触发写盘。返回 `200`（`RuleImportSummary`）：
...
```json
{
  ...
  "failed": 1,
  "results": [
    { "name": "ssh", "finalName": "ssh", "status": "created" },
    { "name": "db",  "status": "failed", "error": "同名已存在" }
  ]
}
```
...
| 字段 | 类型 | 说明 |
|---|---|---|
| `applied` | int | 成功落库条数（created/overwritten/renamed 合计） |
| `skipped` | int | `skip` 动作条数 |
| `failed` | int | 失败条数 |
| `results` | array | 逐条结果 `RuleImportResult`（见下） |
...
|---|---|---|
| `name` | string | 原始名 |
| `finalName` | string | 落库最终名（`rename` 后为新名；`skipped`/`failed` 为空） |
| `status` | string | `created` / `overwritten` / `renamed` / `skipped` / `failed` |
| `error` | string | 失败原因（如 `"同名已存在"`、`"新名称已存在"`、`"本地端口冲突:…"`、`"未知动作:…"`） |

各动作语义：`create` 撞名即 `failed`（`"同名已存在"`）；`overwrite` 命中则替换、未命中则新建（`created`）；`rename` 要求非空且不等原名、新名不得已存在；访客项还会做本地端口（协议族+`bindAddr:bindPort`）冲突校验（跨实例占用或本批次重复均判 `failed`）。配置不存在 `404 / config_not_found`。

...
const (
	defaultRepo       = "mia-clark/frpc-manager"
	defaultInstallSh  = "https://raw.githubusercontent.com/mia-clark/frpc-manager/main/scripts/install.sh"
	defaultInstallPs1 = "https://raw.githubusercontent.com/mia-clark/frpc-manager/main/scripts/install.ps1"
	cacheTTL          = time.Hour
...
type Config struct {
	// Repo is the "owner/name" GitHub repo. Defaults to mia-clark/frpc-manager.
	Repo string
	...
	// overridable via FRPCMGR_INSTALL_SH_URL / FRPCMGR_INSTALL_PS1_URL.
	InstallShURL  string
...
func New(cfg Config) *Updater {
	...
	if cfg.InstallShURL == "" {
		cfg.InstallShURL = env("FRPCMGR_INSTALL_SH_URL", defaultInstallSh)
	}
	if cfg.InstallPs1URL == "" {
		cfg.InstallPs1URL = env("FRPCMGR_INSTALL_PS1_URL", defaultInstallPs1)
	}
	return &Updater{
		cfg:  cfg,
...
func (u *Updater) fetchOne(ctx context.Context, url string) (*Release, error) {
	...
	req.Header.Set("User-Agent", "frpcmgrd-selfupdate")

      ...
      >
        <div
          style={{
            ...
            padding: '32px 28px',
            background:
              'linear-gradient(135deg, #0f172a 0%, #1e1b4b 30%, #312e81 60%, #0891b2 100%)',
            color: '#fff',
            overflow: 'hidden',
          }}
        >
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            ...
            >
              一个守护进程同时托管 N 份 FRPC 配置，每份跑独立 frpc 实例。提供完整的 REST + WebSocket API、9 分组全参数可视化表单、TOML 直编、实时事件流、运行时监控与日志聚合。内嵌 frp{' '}
              <Text strong style={{ color: '#fff' }}>{version.frp || '—'}</Text>，单 Go 二进制（无 cgo）。配套服务端管理器：{' '}
              <a href={SIBLING_REPO_FRPS} target="_blank" rel="noopener noreferrer" style={{ color: '#67e8f9', textDecoration: 'underline' }}>
                FRPS Manager
              </a>。
            </Paragraph>
      ...
      >
        <Tabs
          ...
          items={[
            ...
            {
              key: 'fmc',
              label: <Space size={6}><ToolOutlined />fmc 命令</Space>,
              children: renderFmcTab({ token, copyText }),
            },
            {
              key: 'env',
...
FRPCMGR_API_TOKEN=$(openssl rand -hex 32) ./frpcmgrd serve`;
  ...
  return (
    <div style={{ padding: 18 }}>
      ...
      <SectionTitle>从旧版 frpmgrd 迁移到 frpcmgrd</SectionTitle>
      <Alert
        type="success"
        showIcon
        message="独立子命令，与 install/update 完全解耦"
        description={<span style={{ fontSize: 12.5 }}>在已装好新版后任意时刻执行；自动停旧服务、迁移数据目录、把 FRPMGR_* 环境变量改为 FRPCMGR_*、清理旧二进制，并启动新 frpcmgrd。</span>}
        style={{ marginBottom: 12 }}
      />
      <CodeBlock code={upgradeLegacy} token={token} onCopy={copyText} language="sh" />
...
services:
  frpcmgrd:
    image: ${DOCKER_IMAGE}
    container_name: frpcmgrd
    # host 模式让 frpc 客户端到 frps 的连接直接走宿主机网络（必需）
    network_mode: host
    restart: unless-stopped
    environment:
...
FRPCMGR_API_TOKEN=change-me-to-a-real-strong-token`;
  ...
  return (
    <div style={{ padding: 18 }}>
      <Alert
        ...
        message={
          <Space wrap>
            ...
            <Tag color="blue">linux/arm/v7</Tag>
            <Text code copyable={{ text: DOCKER_IMAGE, onCopy: () => copyText(DOCKER_IMAGE) }}>{DOCKER_IMAGE}</Text>
          </Space>
        }
        description={<span style={{ fontSize: 12 }}>每次推送到 main 与每个发布 tag 自动构建。推荐 host 网络让 frpc 直连 frps，省一层 NAT。</span>}
        style={{ marginBottom: 16 }}
      />
      ...
      <Alert
        ...
        description={
          <span style={{ fontSize: 12.5 }}>
            升级、重装时只要保留挂载的 <Text code>./frpcmgr-data</Text> 卷（容器内 <Text code>/data</Text>），所有隧道配置（<Text code>profiles/</Text>）、日志（<Text code>logs/</Text>）、frp visitor 状态（<Text code>stores/</Text>）、实例元数据（<Text code>meta.json</Text>）都不会丢。
          </span>
        }
      />
    </div>
...
// ---- 4. fmc 命令 ----
function renderFmcTab(opts: { token: TokenLike; copyText: (s: string) => void }) {
  const { token, copyText } = opts;

  const fmcTable: Array<{ category: string; cmd: string; desc: string }> = [
    { category: '服务管理', cmd: 'fmc start',           desc: '启动服务' },
    { category: '服务管理', cmd: 'fmc stop',            desc: '停止服务' },
    { category: '服务管理', cmd: 'fmc restart',         desc: '重启服务' },
    { category: '服务管理', cmd: 'fmc status',          desc: '查看运行状态' },
    { category: '服务管理', cmd: 'fmc logs -f',         desc: '实时跟踪日志（不加 -f 看最近若干行）' },
    { category: '服务管理', cmd: 'fmc enable',          desc: '设置开机自启' },
    { category: '服务管理', cmd: 'fmc disable',         desc: '取消开机自启' },
    { category: '信息查看', cmd: 'fmc info',            desc: '⭐ 完整运行信息（地址/令牌/路径/状态）— 忘了令牌看这个' },
    { category: '信息查看', cmd: 'fmc config',          desc: '查看配置文件（config edit 用编辑器打开）' },
    { category: '信息查看', cmd: 'fmc version',         desc: '显示版本信息' },
    { category: '安装维护', cmd: 'fmc install',         desc: '重新安装（参数透传给 install.sh / install.ps1）' },
    { category: '安装维护', cmd: 'fmc update',          desc: '更新到最新版（保留端口/令牌/数据）' },
    { category: '安装维护', cmd: 'fmc upgrade-legacy',  desc: '🚀 一键迁移旧版 frpmgrd 部署到 frpcmgrd（独立幂等，可随时执行）' },
    { category: '安装维护', cmd: 'fmc uninstall',       desc: '卸载' },
    { category: '帮助',     cmd: 'fmc help',            desc: '显示本帮助' },
  ];
  ...
  return (
    <div style={{ padding: 18 }}>
      <Alert
        type="info"
        showIcon
        message="fmc 是一键安装脚本生成的统一管理命令（已加入 PATH），自动适配 systemd / OpenRC / launchd / Windows 服务。三端一致 15 个子命令。"
        style={{ marginBottom: 16 }}
      />
      <Table
        size="small"
        ...
        columns={[
          {
            ...
            filters: [...new Set(fmcTable.map((r) => r.category))].map((c) => ({ text: c, value: c })),
          ...
          {
            title: '命令', dataIndex: 'cmd', key: 'cmd', width: 220,
            render: (v: string) => (
              <Space size={4}>
      ...
      <Paragraph type="secondary" style={{ marginTop: 14, marginBottom: 0, fontSize: 12 }}>
        Windows 同样提供 <Text code>fmc</Text>（在 PowerShell 或 cmd 中执行；安装目录已加入系统 PATH，新开终端生效）。原生命令也行：systemd 用 <Text code>systemctl status frpcmgrd</Text>；macOS 用 <Text code>launchctl list | grep frpcmgrd</Text>；Windows 用 <Text code>services.msc</Text>。
      </Paragraph>
      <Divider style={{ margin: '14px 0' }} />
      <Title level={5} style={{ margin: '0 0 8px' }}>忘了 API 令牌？</Title>
      <CodeBlock code="fmc info" token={token} onCopy={copyText} language="sh" />
    </div>
  );
}
  ...
  return (
    <div style={{ padding: 18 }}>
      <Alert
        type="info"
        showIcon
        message="所有环境变量统一以 FRPCMGR_ 前缀（旧版 FRPMGR_ 已弃用，升级时用 fmc upgrade-legacy 自动迁移）。一键安装后写入 /etc/frpcmgrd/frpcmgrd.env（Linux）/ launchd plist（macOS）；Docker 通过 -e 或 environment: 块传入。"
        style={{ marginBottom: 16 }}
      />
      <Table
        size="small"
      ...
      <Descriptions column={1} size="small" bordered labelStyle={{ width: 130, background: token.colorFillTertiary }}>
        <Descriptions.Item label="Linux">
          配置：<Text code>/etc/frpcmgrd/frpcmgrd.env</Text> ｜ 数据：<Text code>/var/lib/frpcmgrd/</Text>
        </Descriptions.Item>
        <Descriptions.Item label="macOS">
          配置：写在 launchd plist ｜ 数据：<Text code>/usr/local/var/frpcmgrd/</Text>
        </Descriptions.Item>
        <Descriptions.Item label="Windows">
          数据：<Text code>%ProgramData%\frpcmgrd\data\</Text>
        </Descriptions.Item>
